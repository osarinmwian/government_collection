...
public class BuyPowerGateway : IPaymentGateway
{
    private readonly HttpClient _httpClient;
    private readonly BuyPowerSettings _settings;
    private readonly ILogger<BuyPowerGateway> _logger;

    public BuyPowerGateway(HttpClient httpClient, BuyPowerSettings settings, ILogger<BuyPowerGateway> logger)
    {
    ...
    public async Task<BillInquiryResponseDto> InquireBillAsync(BillInquiryDto request)
    {
        try
        {
            ...
            var response = await _httpClient.PostAsync("buypower/bill-inquiry", content);
        ...
        {
            _logger.LogError(ex, "Error during BuyPower bill inquiry");
    ...
    public async Task<PaymentResponseDto> ProcessPaymentAsync(PaymentRequestDto request)
    {
        try
        {
            ...
            var response = await _httpClient.PostAsync("buypower/process-payment", content);
            var responseContent = await response.Content.ReadAsStringAsync();
        ...
        {
            _logger.LogError(ex, "Error during BuyPower payment processing");
    ...
    public async Task<PaymentResponseDto> VerifyPaymentAsync(string transactionReference)
    {
        try
        {
            var response = await _httpClient.GetAsync($"buypower/verify-payment/{transactionReference}");
            var responseContent = await response.Content.ReadAsStringAsync();
        ...
        {
            _logger.LogError(ex, "Error during BuyPower payment verification");
            return new PaymentResponseDto { Status = "Failed", Message = "Service unavailable" };
        }
    }
}